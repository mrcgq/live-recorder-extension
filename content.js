 ...
 * 2. 监听全屏切换，自适应将录制栏挂载至全屏顶层，防止被遮挡 [纠正 2]
 * 3. 监听跨进程主动唤醒指令 (start_recording_now)，实现一键秒录 [纠正 3]
 */
...
function isLiveVideo(video) {
  ...
  if (rect.width < 160 || rect.height < 90) return false; // 宽容化边界，适配部分迷你悬浮播放器
...
// 物理加固：支持向上进行 5 级祖先层析扫描，彻底击穿播放器透明遮罩（Mask）
...
function createHoverBar(video) {
  ...
  // 物理加固：全屏穿透挂载。如果当前处于全屏，自动挂载到全屏 DOM 顶层，防止被播放器遮挡 [纠正 2]
...
// 物理加固：监听来自 Popup 或小窗控制台的“主动唤醒开始直录”信号 [纠正 3]
    ...
    case 'start_recording_now':
      (async () => {
        ...
        if (video) {
          ...
          sendResponse({ ok: false, error: '未在页面上检测到正在播放的直播视频源' });
        }
      })();
      return true; // 保持通道异步存活
